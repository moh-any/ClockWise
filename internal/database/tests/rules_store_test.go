...
func TestGetRulesByOrganizationID(t *testing.T) {
	...
	orgID := uuid.New()
	query := regexp.QuoteMeta(`SELECT organization_id, shift_max_hours, shift_min_hours, max_weekly_hours, min_weekly_hours, fixed_shifts, number_of_shifts_per_day, meet_all_demand, min_rest_slots, slot_len_hour, min_shift_length_slots FROM organizations_rules WHERE organization_id = $1`)
	...
	t.Run("NotFound", func(t *testing.T) {
		mock.ExpectQuery(query).WithArgs(orgID).WillReturnError(sql.ErrNoRows)
		rules, err := store.GetRulesByOrganizationID(orgID)
		assert.NoError(t, err)
		assert.Nil(t, rules)
		AssertExpectations(t, mock)
