...
def create_sample_data() -> SchedulerInput:
    """
    Create sample scheduling data for testing.
    
    Returns:
        SchedulerInput object with sample employees, roles, and demand
    """
    # Define roles
    roles = [
        Role(id='cashier', producing=False, items_per_hour=0, min_present=0, is_independent=True),
        Role(id='prep', producing=False, items_per_hour=0, min_present=0, is_independent=False),
        Role(id='cook', producing=False, items_per_hour=0, min_present=0, is_independent=False),
        Role(id='server', producing=True, items_per_hour=30, min_present=1, is_independent=True),
    ]
    
    # Define production chains (prep -> cook)
    chains = [
        ProductionChain(id='kitchen', roles=['prep', 'cook'], contrib_factor=1.0)
    ]
    
    # Define employees - 27 total (3 eligible for prep, all eligible for server)
    employees = [
        Employee(id='alice', wage=15, max_hours_per_week=40, max_consec_slots=8, pref_hours=32,
                 role_eligibility={'prep', 'server'}),
        Employee(id='bob', wage=20, max_hours_per_week=40, max_consec_slots=8, pref_hours=40,
                 role_eligibility={'prep', 'server'}),
        Employee(id='charlie', wage=15, max_hours_per_week=40, max_consec_slots=8, pref_hours=32,
                 role_eligibility={'prep', 'server'}),
    ]
    
    # Add 24 more diana employees (only eligible for server)
    for i in range(24):
        suffix = str(i + 2) if i > 0 else ''
        emp_id = f'diana{suffix}' if i > 0 else 'diana'
        employees.append(
            Employee(id=emp_id, wage=18, max_hours_per_week=40, max_consec_slots=8, pref_hours=36,
                     role_eligibility={'server'})
        )
    
    # Define demand (7 days x 12 slots)
    num_days = 7
    num_slots_per_day = 12
    demand = {}
    
    # Sample demand pattern (varies by time of day)
    for d in range(num_days):
        for t in range(num_slots_per_day):
            if t < 3:  # Morning: low demand
                demand[(d, t)] = 15
            elif t < 6:  # Midday: high demand
                demand[(d, t)] = 30
            elif t < 9:  # Afternoon: medium demand
                demand[(d, t)] = 25
            else:  # Evening: medium demand
                demand[(d, t)] = 20
    ...
    return SchedulerInput(
        ...
        chains=chains,
        slot_len_hour=1.0,
        min_rest_slots=1,  # Reduced from 2 to allow slot 0 coverage
...
def main():
    ...
    # Solve using the reusable function
    print("\nBuilding model...")
    print("Solving...")
    solution, description, insights = solve_schedule(data, time_limit_seconds=30)
    ...
    if insights and solution:
        ...
        if insights['hiring_recommendations']:
            ...
            for rec in insights['hiring_recommendations'][:3]:  # Top 3
                print(f"  [{rec['priority'].upper()}] Hire {rec['recommended_hires']} {rec['role']}(s)")
        ...
        print(f"  Range: {wb['min_hours']:.1f}h - {wb['max_hours']:.1f}h")
        print(f"  Unused: {wb['unused_employees']}, Underutilized: {wb['underutilized_employees']}, "
              f"Well-utilized: {wb['well_utilized_employees']}, Overutilized: {wb['overutilized_employees']}")
        print(f"  Balance score: {wb['balance_score']:.2%}")
        
        # Cost analysis
        print("\n--- Cost Analysis ---")
        ca = insights['cost_analysis']
        print(f"  Total wage cost: ${ca['total_wage_cost']:.2f}")
        if ca['opportunity_cost_unmet_demand'] > 0:
            print(f"  Opportunity cost (unmet demand): ${ca['opportunity_cost_unmet_demand']:.2f}")
            print(f"  Total cost: ${ca['total_cost']:.2f}")
        
        # Coverage gaps
        if insights['coverage_gaps']:
            print(f"\n--- Coverage Gaps ({len(insights['coverage_gaps'])} slots) ---")
            for gap in insights['coverage_gaps'][:3]:  # Show worst 3
                print(f"  Day {gap['day']}, Slot {gap['slot']}: {gap['coverage_rate']:.1%} coverage "
                      f"({gap['employees_working']} employees) [{gap['severity']}]")
